...
def run(displaymode):
    ...
    if (displaymode == 'left network only') or (displaymode == 'right network only'):
        ...
        response['separator'] = '\t'
        response['url'] = 'service/lineupdataset'
        response['columns'] = [ {'column': 'entity', 'type': 'string'},{'column': 'LSGM','type':'number', 'domain':[0,1]},{'column': 'lev','type':'number', 'domain':[0,1]},{'column': 'substring','type':'number', 'domain':[0,1]}, {'column': '1hop','type':'number', 'domain':[0,1]}, {'column': '2hop','type':'number', 'domain':[0,1]}]
        response['layout'] = {'primary': [   {'column': 'entity', 'width':100},{'column': 'LSGM','width':75},{'column': 'lev','width':75}, {'column': 'substring','width':75},  {'column': '1hop','width':50}, {'column': '2hop','width':50}, {"type": "stacked","label": "Combined", "children": [{'column': 'lev','width':50}, {'column': 'substring','width':50}, {'column': '1hop','width':50}, {'column': '2hop','width':50}]}]}
