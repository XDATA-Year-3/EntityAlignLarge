...
*/
  ...
  // CRL - modified to handle case of dynamic data load from Tangelo service 
  ...
  function loadDataset(ds) {
    function loadDesc(desc, baseUrl) {
      ...
      // added for tangelo linkage  
      } else if (desc.url) {
        console.log('loading from',desc.url)
         d3.json(desc.url+ '/'+ 'localhost/topk' , function (_data) {
          loadDataImpl(name, desc, _data);
        });
      }
    }
    // add special type for this integration
    if (ds.descriptionFile == 'topk') {
      var name = ds.descriptionFile.substring(0, ds.descriptionFile.length - 5);
      d3.json(ds.baseURL , function (desc) {
        loadDesc(desc, ds.baseURL);
      })
    } else if (ds.descriptionFile) {
      var name = ds.descriptionFile.substring(0, ds.descriptionFile.length - 5);
      d3.json(ds.baseURL + "/" + ds.descriptionFile, function (desc) {
        loadDesc(desc, ds.baseURL);
      })
